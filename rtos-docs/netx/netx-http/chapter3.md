...
이 챕터에서는 아래에 나열된 모든 NetX HTTP 서비스에 대해 알파벳 순서로 설명하고 있습니다.
...
- nx_http_client_create *HTTP 클라이언트 인스턴스 만들기*
- nx_http_client_delete *HTTP 클라이언트 인스턴스 삭제*
- nx_http_client_get_start *HTTP GET 요청 시작*
- nx_http_client_get_start_extended *HTTP GET 요청 시작*
- nx_http_client_get_packet *다음 리소스 데이터 패킷 가져오기*
- nx_http_client_put_start *HTTP PUT 요청 시작*
- nx_http_client_put_start_extended *HTTP PUT 요청 시작*
- nx_http_client_put_packet *다음 리소스 데이터 패킷 보내기*
- *nx_http_client_set_connect_port* *HTTP 서버에 연결할 포트 변경*

**HTTP 서버 서비스:**

- nx_http_server_cache_info_callback_set *지정된 URL의 기간 및 마지막으로 수정한 날짜를 검색하는 콜백 설정*
- nx_http_server_callback_data_send *콜백 함수에서 HTTP 데이터 보내기*
- nx_http_server_callback_generate_response_header *콜백 함수에서 응답 헤더 만들기*
- nx_http_server_callback_generate_response_header_extended *콜백 함수에서 응답 헤더 만들기*
- nx_http_server_callback_packet_send *HTTP 콜백에서 HTTP 패킷 보내기*
- nx_http_server_callback_response_send *콜백 함수에서 응답 보내기*
- nx_http_server_callback_response_send_extended *콜백 함수에서 응답 보내기*
- nx_http_server_content_get *요청에서 콘텐츠 가져오기*
- nx_http_server_content_get_extended *요청에서 콘텐츠 가져오기, 빈(콘텐츠 길이 0) 요청 지원*
- nx_http_server_content_length_get *요청에서 콘텐츠 길이 가져오기*
- nx_http_server_content_length_get_extended *요청에서 콘텐츠 길이 가져오기, 빈(콘텐츠 길이 0) 요청 지원*
- nx_http_server_create *HTTP 서버 인스턴스 만들기*
- nx_http_server_delete *HTTP 서버 인스턴스 삭제*
- nx_http_server_get_entity_content *URL에서 엔터티 콘텐츠의 크기 및 위치 반환*
- nx_http_server_get_entity_header *URL 엔터티 헤더를 지정된 버퍼로 추출*
- nx_http_server_gmt_callback_set *GMT 날짜 및 시간을 검색하는 콜백 설정*
- nx_http_server_invalid_userpassword_notify_set *클라이언트 요청에서 잘못된 사용자 이름과 암호를 받은 경우의 콜백 설정*
- nx_http_server_mime_maps_additional_set *HTML에 대한 추가 MIME 맵 정의*
- nx_http_server_packet_content_find *HTTP 헤더에서 콘텐츠 길이 추출 및 콘텐츠 데이터 시작에 대한 포인터 설정*
- nx_http_server_packet_get *클라이언트 패킷 직접 받기*
- nx_http_server_param_get *요청에서 매개 변수 가져오기*
- nx_http_server_query_get *요청에서 쿼리 가져오기*
- nx_http_server_start *HTTP 서버 시작*
- nx_http_server_stop *HTTP 서버 중지*
- nx_http_server_type_get *헤더에서 HTTP 형식(예: 텍스트/일반) 추출*
- nx_http_server_type_get_extended *헤더에서 HTTP 형식(예: 텍스트/일반) 추출*
- nx_http_server_digest_authenticate_notify_set *다이제스트 인증 콜백 함수 설정*
- nx_http_server_authentication_check_set *인증 확인 콜백 함수 설정*

## <a name="nx_http_client_create"></a>nx_http_client_create

### <a name="create-an-http-client-instance"></a>HTTP 클라이언트 인스턴스 만들기
...
이 서비스는 지정된 IP 인스턴스에서 HTTP 클라이언트 인스턴스를 만듭니다.
...
- NX_HTTP_POOL_ERROR (0xE9) 패킷 풀의 잘못된 페이로드 크기
...
### <a name="delete-an-http-client-instance"></a>HTTP 클라이언트 인스턴스 삭제
...
### <a name="start-an-http-get-request"></a>HTTP GET 요청 시작
...
이 서비스는 이전에 만든 HTTP 클라이언트 인스턴스의 "리소스" 포인터에서 지정한 리소스를 가져오려고(GET) 시도 합니다. 이 루틴에서 NX_SUCCESS를 반환하는 경우 애플리케이션에서 *nx_http_client_get_packet* 을 여러 번 호출하여 요청된 리소스 콘텐츠에 해당하는 데이터 패킷을 검색할 수 있습니다.
...
이 서비스는 더 이상 사용되지 않습니다. 개발자는 *nx_http_client_get_start_extended()* 로 마이그레이션하는 것이 좋습니다.
...
### <a name="get-next-resource-data-packet"></a>다음 리소스 데이터 패킷 가져오기
...
이 서비스는 이전 *nx_http_client_get_start* 호출에서 요청한 리소스 콘텐츠의 다음 패킷을 검색합니다. 이 루틴에 대한 연속 호출은 NX_HTTP_GET_DONE의 반환 상태를 받을 때까지 수행해야 합니다.
...
### <a name="start-an-http-put-request"></a>HTTP PUT 요청 시작 
...
### <a name="send-next-resource-data-packet"></a>다음 리소스 데이터 패킷 보내기
...
이 서비스는 리소스 콘텐츠의 다음 패킷을 HTTP 서버에 보내려고 시도합니다. 이 루틴은 보낸 패킷의 결합된 길이가 이전 *nx_http_client_put_start()* 호출에 지정된 "total_bytes"와 같을 때까지 반복적으로 호출해야 합니다.
...
- **NX_HTTP_NOT_READY** (0xEA) HTTP 클라이언트가 준비되지 않음
- **NX_HTTP_REQUEST_UNSUCCESSFUL_CODE** (0xEE) ** 서버 오류 코드를 받음 -**NX_HTTP_BAD_PACKET_LENGTH** (0xED) 잘못된 패킷 길이
- **NX_HTTP_AUTHENTICATION_ERROR** (0xEB) 잘못된 이름 및/또는 암호
...
### <a name="set-the-connection-port-to-the-server"></a>서버에 대한 연결 포트 설정
...
- **NX_SUCCESS** (0x00) 연결 포트를 성공적으로 변경함
- **NX_INVALID_PORT** (0x46) 포트가 최댓값(0xFFFF)을 초과하거나 0임
- NX_PTR_ERROR (0x07) 잘못된 포인터 입력
...
### <a name="set-the-callback-to-retrieve-url-max-age-and-date"></a>URL 최대 기간 및 날짜를 검색하는 콜백 설정
...
이 서비스는 지정된 리소스의 최대 기간 및 마지막으로 수정한 날짜를 가져오기 위해 호출되는 콜백 서비스를 설정합니다.
...
### <a name="send-data-from-callback-function"></a>콜백 함수에서 데이터 보내기
...
이 서비스는 애플리케이션의 콜백 루틴에서 제공된 패킷의 데이터를 보냅니다. 일반적으로 GET/POST 요청과 연결된 동적 데이터를 보내는 데 사용됩니다. 이 함수를 사용할 경우 콜백 루틴에서 전체 응답을 적절한 형식으로 보내di 합니다. 또한 콜백 루틴에서 NX_HTTP_CALLBACK_COMPLETED 상태를 반환해야 합니다.

**입력 매개 변수**

- **server_ptr** HTTP 서버 제어 블록에 대한 포인터
- **data_ptr** 보내는 데이터에 대한 포인터
- **data_length** 보내는 바이트의 수
...
### <a name="create-a-response-header-in-a-callback-function"></a>콜백 함수에서 응답 헤더 만들기
...
이 서비스는 HTTP 서버에서 클라이언트 GET, PUT 및 DELETE 요청에 응답할 때 _ *nx_http_server_generate_response_header* 내부 함수를 호출합니다. HTTP 서버 애플리케이션에서 클라이언트에 대한 응답을 설계하는 경우 HTTP 서버 콜백 함수에서 사용하기 위한 것입니다.
...
### <a name="send-an-http-packet-from-callback-function"></a>콜백 함수에서 HTTP 패킷 보내기
...
### <a name="send-response-from-callback-function"></a>콜백 함수에서 응답 보내기
...
이 서비스는 애플리케이션의 콜백 루틴에서 제공된 응답 정보를 보냅니다. 일반적으로 GET/POST 요청과 연결된 사용자 지정 응답을 보내는 데 사용됩니다. 이 함수를 사용하는 경우 콜백 루틴에서 NX_HTTP_CALLBACK_COMPLETED 상태를 반환해야 합니다.

이 서비스는 더 이상 사용되지 않습니다. 개발자는 *nx_http_server_callback_response_send_extended()* 로 마이그레이션하는 것이 좋습니다.
...
이 서비스는 *nx_http_server_callback_response_send()* 를 대체합니다. 이 버전은 길이 정보를 입력 인수로 사용합니다.
...
### <a name="get-content-from-the-request"></a>요청에서 콘텐츠 가져오기
...
### <a name="get-content-from-the-requestsupports-zero-length-content-length"></a>요청에서 콘텐츠 가져오기/길이가 0인 콘텐츠 길이 지원
...
이 서비스는 *nx_http_server_content_get()* 과 거의 동일하며, POST 또는 PUT HTTP 클라이언트 요청에서 지정된 양의 콘텐츠를 검색하려고 시도합니다. 그러나 콘텐츠 길이가 0인 요청('빈 요청')을 유효한 요청으로 처리합니다. HTTP 서버를 만드는 동안 지정된 애플리케이션의 요청 알림 콜백(*nx_http_server_create()* )에서 호출해야 합니다.
...
### <a name="get-length-of-content-in-the-request"></a>요청에서 콘텐츠 길이 가져오기
...
### <a name="get-length-of-content-in-the-requestsupports-content-length-of-zero-value"></a>요청에서 콘텐츠 길이 가져오기/0 값의 콘텐츠 길이 지원
...
### <a name="create-an-http-server-instance"></a>HTTP 서버 인스턴스 만들기
...
### <a name="delete-an-http-server-instance"></a>HTTP 서버 인스턴스 삭제
...
### <a name="retrieve-the-location-and-length-of-entity-data"></a>엔터티 데이터의 위치 및 길이 검색
...
자세한 내용은 *nx_http_server_get_entity_header* 를 참조하세요.
...
- **NX_HTTP_BOUNDARY_ALREADY_FOUND** (0xF4) HTTP 서버 내부 다중 파트 표식에 대한 콘텐츠가 이미 있음
- NX_HTTP_ERROR (0xE0) HTTP 서버 내부 오류
- NX_PTR_ERROR (0x07) 잘못된 포인터 입력
...
### <a name="retrieve-the-contents-of-entity-header"></a>엔터티 헤더의 콘텐츠 검색
...
- **NX_HTTP_NOT_FOUND (0xE6)** 엔터티 헤더 필드가 없음
- **NX_HTTP_TIMEOUT (0xE1)** 다중 패킷 클라이언트 메시지에 대한 다음 패킷을 받는 시간이 만료됨 
- NX_PTR_ERROR (0x07) 잘못된 포인터 입력
- NX_CALLER_ERROR (0x11) 이 서비스의 잘못된 호출자
...
### <a name="set-the-callback-to-obtain-gmt-date-and-time"></a>GMT 날짜 및 시간을 가져오는 콜백 설정
...
이 서비스는 이전에 만든 HTTP 서버에서 GMT 날짜 및 시간을 가져오는 콜백을 설정합니다. 이 서비스는 HTTP 서버에서 헤더를 클라이언트에 대한 HTTP 서버 응답에 만들 때 호출됩니다.
...
### <a name="set-the-callback-to-to-handle-invalid-userpassword"></a>잘못된 사용자/암호를 처리하는 콜백 설정
...
### <a name="set-additional-mime-maps-for-html"></a>HTML에 대한 추가 MIME 맵 설정 
...
이 서비스를 통해 HTTP 애플리케이션 개발자는 NetX HTTP 서버에서 제공하는 기본 MIME 형식에서 추가 MIME 형식을 추가할 수 있습니다(정의된 형식 목록은 *nx_http_server_get_type* 참조).
...
요청 알림 함수가 HTTP 서버에 등록된 경우 요청 알림 콜백에서 *nx_http_server_type_get* 을 호출하여 파일 형식을 구문 분석할 수 있습니다.
...
### <a name="extract-content-length-and-set-pointer-to-start-of-data"></a>콘텐츠 길이 추출 및 데이터 시작에 대한 포인터 설정
...
이 서비스는 엔터티 헤더 앞의 선행 포인터를 수정하므로 *nx_http_server_get_entity_header()* 를 호출하기 전에 호출하지 않아야 합니다.
...
### <a name="receive-the-next-http-packet"></a>다음 HTTP 패킷 받기
...
### <a name="get-parameter-from-the-request"></a>요청에서 매개 변수 가져오기
...
### <a name="get-query-from-the-request"></a>요청에서 쿼리 가져오기
...
### <a name="start-the-http-server"></a>HTTP 서버 시작
...
- **NX_SUCCESS** (0x00) HTTP 서버를 성공적으로 시작함
- NX_PTR_ERROR (0x07) 잘못된 포인터 입력
...
### <a name="stop-the-http-server"></a>HTTP 서버 중지
...
- **NX_SUCCESS** (0x00) HTTP 서버를 성공적으로 중지함
- NX_PTR_ERROR (0x07) 잘못된 포인터 입력
- NX_CALLER_ERROR (0x11) 이 서비스의 잘못된 호출자
...
### <a name="extract-file-type-from-client-http-request"></a>클라이언트 HTTP 요청에서 파일 형식 추출
...
이 서비스는 *http_type_string* 버퍼에서 HTTP 요청 유형을 추출하고, *name* 입력 버퍼(일반적으로 URL)의 반환 값에서 해당 길이를 추출합니다. MIME 맵이 없는 경우 기본적으로 "텍스트/일반" 형식으로 설정됩니다. 그렇지 않으면 추출된 형식을 HTTP 서버 기본 MIME 맵과 비교하여 일치시킵니다. NetX HTTP 서버의 기본 MIME 맵은 다음과 같습니다.
...
제공되면 사용자 정의 추가 MIME 맵 세트도 검색합니다. 사용자 정의 맵에 대한 자세한 내용은 *nx_http_server_mime_maps_addtional_set()* 를 참조하세요.
...
*nx_http_server_callback_generate_response_header* 를 참조하세요.
...
이 서비스는 *http_type_string* 버퍼에서 HTTP 요청 유형을 추출하고, *name* 입력 버퍼(일반적으로 URL)의 반환 값에서 해당 길이를 추출합니다. MIME 맵이 없는 경우 기본적으로 "텍스트/일반" 형식으로 설정됩니다. 그렇지 않으면 추출된 형식을 HTTP 서버 기본 MIME 맵과 비교하여 일치시킵니다. NetX Duo HTTP 서버의 기본 MIME 맵은 다음과 같습니다.
...
*http_type_string* 버퍼의 크기

**반환 값**

- **문자열 길이(바이트)** 0이 아닌 값은 성공<br />0은 오류를 나타냄
...
### <a name="set-digest-authenticate-callback-function"></a>다이제스트 인증 콜백 함수 설정
...
- **http_server_ptr** HTTP 서버 인스턴스에 대한 포인터
- **digest_authenticate_callback** 다이제스트 인증 콜백에 대한 포인터
...
### <a name="set-authentication-checking-callback-function"></a>인증 확인 콜백 함수 설정
...
- **http_server_ptr** HTTP 서버 인스턴스에 대한 포인터
- **authentication_check_extended** 애플리케이션의 인증 확인에 대한 포인터
