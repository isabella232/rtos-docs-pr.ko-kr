...
이 장에서는 아래에 나열된 모든 NetX Duo DHCP 서버 서비스에 대해 알파벳 순서로 설명하고 있습니다.
...
- **stack_size DHCP 서버 스택의 크기** input_address_list 서버의 IP 주소 목록에 대한 포인터
- **name_ptr DHCP 서버 이름에 대한 포인터** packet_pool_ptr DHCP 서버 패킷 풀에 대한 포인터
