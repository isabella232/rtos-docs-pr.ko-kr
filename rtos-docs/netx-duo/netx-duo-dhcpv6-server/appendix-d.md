...
ms.locfileid: "116783682"
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a>부록 D - Azure RTOS NetX Duo Advanced DHCPv6 서버 예제

이 서버는 RFC 3315에서 요구하는 대로 비휘발성 메모리의 서버 IP 주소 임대 테이블 및 클라이언트 레코드 테이블을 저장하고 검색하는 방법을 보여 주는 고급 DHCPv6 서버입니다.

이 예제에서 포함 파일 *nxd_dhcpv6_server.h* 는 7줄에서 가져옵니다. 그런 다음, 아래 예제 코드에서 NetX Duo DHCPv6 서버 애플리케이션 스레드가 81줄에 만들어집니다. DHCPv6 제어 블록 “*dhcp_server_0*”은 이전에 19줄의 전역 변수로 정의되었습니다.

NetX Duo DHCPv6 서버 인스턴스를 만들기 전에 데모에서는 84줄에 DHCPv6 메시지를 보내기 위한 패킷 풀을 만들고, 102줄에서 IP 스레드 인터페이스를 만들고, 116줄의 NetX Duo에서 UDP를 사용하도록 설정합니다.

136줄에서 NetX Duo DHCPv6 서버를 성공적으로 만들면 1장에서 설명된 두 개의 옵션 콜백 함수를 포함합니다. 이는 NetX Duo가 162-163줄에서 IPv6 및 DHCPv6 작업을 처리하는데 필요한 IPv6 및 ICMPv6을 사용하도록 설정합니다. DHCPv6 서버 스레드를 실행할 준비가 되기 전에 DHCPv6 서버는 해당 IPv6 주소(167-180)의 유효성을 검사하고, 208-209줄에서 해당 DHCPv6 인터페이스를 정의해야 합니다. 266줄에서 이전에 만든 서버 DUID가 없는 경우 서버를 만들기 위해 *nx_dhcpv6_set_server_duid* 서비스가 호출됩니다. 서버는 할당 가능한 주소 목록을 만들기 위한 IP 주소 범위를 설정합니다. 이전 세션에서 데이터를 저장하는 경우 메모리에서 283-318줄의 클라이언트 레코드와 IPv6 임대 데이터를 검색합니다. 또한 해당 서버 DUID를 만들거나 이전에 만든 경우 사용자 지정 스토리지에서 DUID 데이터를 검색합니다. 이는 다시 부팅에서 일관적인 서버 DUID를 재현하는 데 필요합니다. 필요에 따라 호스트 애플리케이션은 DNS 서버 구성을 요청하는 클라이언트에 대한 DNS 서버를 정의합니다.

다음으로 호스트는 329줄에서 DHCPv6 서버를 시작합니다. 그러면 DHCPv6 서버 UPD 소켓이 만들어지고 NetX Duo DHCPv6 서버 타이머가 활성화됩니다. 그런 다음, 서버는 클라이언트 요청을 받기 위해 대기합니다. 여러 클라이언트를 처리할 수 있지만 한 번에 하나의 클라이언트 요청만 처리할 수 있습니다.

예제의 나머지 부분에는 할당할 수 있는 IPv6 주소 풀의 서버 테이블과 비휘발성 메모리의 클라이언트 레코드를 각각 저장하고 검색하는 호스트 구현이 포함되어 있습니다. 또한 NetX Duo DHCPv6 서버에서 직접 지원되지 않는 DHCPv6 클라이언트에서 요청하는 옵션에 대한 옵션 처리기가 포함되어 있습니다(현재 DNS 서버 옵션만 지원됨). 마지막으로 서버에서 할당된 IP 임대 만료를 추적하는 '비휘발성 시간'을 저장하고 검색하는 방법을 보여 주는 코드가 있습니다.
...
**그림 6. 고급 NetX Duo DHCPv6 서버 애플리케이션**